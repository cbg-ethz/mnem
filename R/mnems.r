...
#' @param penalized if set to TRUE, the penalized likelihood will be used for the mcmc. 
...
mnem <- function(D, inference = "em", search = "greedy", phi = NULL,
    ...
    if (k == 1) {
            ...
            do_inits <- function(s) {
                if (!is.null(init)) {
                    ...
                    if (nullcomp) {
                        k <- length(res1) + 1
                        res1[[k]] <- list()
                        res1[[k]][["adj"]] <- res1[[i]][["adj"]]*0
                        res1[[k]][["subtopo"]] <- res1[[i]][["subtopo"]]*0
                        mw <- rep(1/k,k)
                    } else {
                        k <- length(res1)
                    }
                          ...
                          count < max_iter)) | time0) & !stop) {
                              ...
                              for (i in seq_len(k)) {
                                  ...
                                  if (!nullcomp | i != k) {
                                      ...
                                      res[[i]] <- list()
                                      res[[i]]$adj <- res[[i-1]]$adj*0
                                      res[[i]]$subtopo <- res[[i-1]]$subtopo*0
                                  }
        ...
        if (inference %in% "mcmc"){
          mcmc <- MCMC(Nems=k, Sgenes=Sgenes, data=data, stop_counter=mcmc_iterations, burn_in=mcmc_burn_in, 
...
simData <- function(Sgenes = 5, Egenes = 1,
    ...
    if (badCells > 0) {
        baddata <- matrix(0,nrow(data),badCells)
        colnames(baddata) <- sample(seq_len(Sgenes),badCells,replace=TRUE)
