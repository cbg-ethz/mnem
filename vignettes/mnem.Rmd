...
The input data is an m times n matrix of log odds (e.g. for fold changes as in
    ...
    M\&NEM can is designed for single cell data and therefore can handle and
...
The M\&NEM package includes a functions for the simulation of typical single
    ...
    Additionally we assume generate the data from a mixture of two components
    ...
    odds by adding Gaussian with mean $1$ to all $1$s and with mean $-1$ to
    all $0$s. Figure 1 shows a heatmap of our generated data.Since we used
...
We "forget" the solution stored in sim$Nem and use the 'mnem' function to infer
   ...
   networks we used to simulate the data. Notice, that our input for the
...
par(mfrow=c(1,2))
plotDnf(simmini$Nem[[1]], nodelabels=list(A="1", B="2", C="3"),
        bordercol="red", main = "Mixture weigth: 20%")
plotDnf(simmini$Nem[[2]], nodelabels=list(A="1", B="2", C="3"),
        bordercol="blue", main = "Mixture weigth: 80%")
```
...
The data object 'sim' contains four lists with results of mixture with
...
The different methods we compared are besides M\&NEM, random drawing of of the
causal network and a naive cluster approach (cNEM). For cNEM we first
...
```
For the code of the latter two (only minmal difference in code) see the R code
    of this vignette.
...
PERTURBSeq (Dixit *et al.*, 2016). Those screen consists of single cell RNAseq
...
```
Figure 9 shows the distribution of responsibilities for all three data sets.
