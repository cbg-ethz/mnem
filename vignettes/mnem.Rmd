...
We forget the solution stored in 'simmini$Nem' and use the 'mnem' function to
   ...
   for our largest component. Interestingly in the red and brown
   networks S-gene 1 is always at the bottom and this similarity leads
   to the fact, that best cell in the red network never reaches a
   responsibility of $100$\% for any cell attached to S-gene 1.
   
   We can speed up the computation with the 'parallel = n' parameter with
   $n$ threads.

```{r, fig.height=6, fig.width=14, fig.cap=fig.cap01}
bestk <- mnemk(data, ks = 1:5, search = "greedy", starts = 10,
                       parallel = NULL)
plot(bestk$best)
```
```{r, fig.cap=fig.cap03}
...
the three component model gives us the best score (minimum). The five
component model is now also overtaken by the two component
...
ground truth) we get the same signaling
networks.
