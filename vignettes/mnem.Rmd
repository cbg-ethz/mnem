...
for i in `seq 1 2`; do
    ...
    echo "R --slave 'Sgenes=3' 'run=$i' 'noise=1:3' 'nem=1:4' < mnem/vignettes/sim.R" >> $file
    qsub -q mpi01.q -pe make 1 $file
    sleep 1
    rm $file
done

for i in `seq 54 100`; do
    for j in `seq 1 3`; do
	...
	echo "module load gcc/5.3.0" >> $file
	echo "module load R/3.4.0" >> $file
	echo "R --slave 'Sgenes=10' 'run=$i' 'noise=$j' 'nem=1:4' < mnem/vignettes/sim.R" >> $file
...
pdf("temp.pdf", width = 10, height = 14)
...
for (i in c(1,3,5,7,9)) {
    for (j in c(3,5,10)) {
        ...
        for (k in c(1,2,4)) {
            for (l in 1:4) {
                tmp2 <- cbind(tmp2, tmp[,l,i,k,3])
            }
        }
        boxplot(tmp2, col = c(rep("red", 4), rep("blue", 4), rep("grey", 4)), xaxt = "n", ylim = c(0,1))
        axis(1, 1:(length(noises)*3), rep(noises, 3))
        axis(1, c(length(noises)/2 + 0.5, length(noises)/2*3 + 0.5, length(noises)/2*5 + 0.5), c("mnem", "nem", "random"), line = 2, tick = FALSE, cex = 2)
