...
(no rownames) by their random/unique patterns. There is a clear
clustering noticeable for all knock-downs hinting at differential
...
of simulated cells. The choses mixture weights almost agree with the
...
We forget the solution stored in 'simmini$Nem' and use the 'mnem' function to
   ...
   independent starts for the EM algorithm. Since the number of components $k$
   is a priori
   not known, we perform optimization for $k=1,2,3,4,5$ and use our penalized
   ...
   networks we used to generate the data. However, mixture weights are
   quite different to our input. E.g. only $34$\% instead of $40$\%
   for our largest component. Interestingly in the red and blue
   networks S-gene 1 is always at the bottom and this similarity leads
   to the fact, that neither network reaches a responsibility of
   $100$\% for any cell.
...
mpen <- mllh <- numeric(5)
for (k in 1:5) {
    mlist[[k]] <- mnem(data, k = k, search = "greedy", starts = 10)
...
(right, red). Notice that the raw log likelihoods of $k=4$ and $k=5$ are higher
than the $k=3$ component model. However, if we penalize complexity
the three component model gives us the best score (minimum). The four
and five component models are now also overtaken by the two component
model due to complexity or lack thereof, respectively.

## Cell responsibilities

Figure 5 shows the actual responsibilities of the best scoring
model. Many cells reach a responsibility of $100\%$. This
is expected due to the relatively low level of noise. However, some cells
have responsibilities around $50$ and $80$\%, respectively. Also
notice, that the intermediate responsibilities are symmetric. This
means two of the three components are more similar to each other and
compete for some cells, while the third components has all cells
exclusively at $100$\%.
...
result_disc <- mnem(datadisc, k = 3, search = "greedy", starts = 10,
                        method = "disc", fpfn = c(0.1,0.1))
plot(result_disc)
```

Apart from different mixture weights (more similar to the
groudn truth) we get the same signaling
networks.
...
Parallel computation on the random restarts to speed up the inference
is available with the parameter setting 'parallel = n' for $n$ threads.

```{r, fig.height=6, fig.width=10, fig.cap=fig.cap62}
result <- mnem(data, k = 3, search = "greedy", multi = TRUE, starts = 10,
               parallel = NULL)
plot(result)
```

M\&NEM infers the correct causal networks for $k=3$ slightly better
mixture weights than without sample with multiple knock-downs.
...
Perturb-seq (Dixit *et al.*, 2016). Those screens consists of single cell RNAseq
