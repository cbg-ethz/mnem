...
for i in `seq 1 100`; do
    ...
    echo "R --slave 'Sgenes=3' 'run=$i' 'noise=1:4' 'nem=9' < mnem/vignettes/sim.R" >> $file
...
for (run in 1:length(acc)) {
    ...
    if (do == 1) {
        ...
        res <- mnem(data, starts = starts,
                    ...
                    initnets = initnets,
                    popSize = popSize,
                    stallMax = stallMax#, k = 1
                    )
        ## getBIC <- function(x, AIC = FALSE) {
        ##     fpar <- 0
        ...
        ##     } else {
        ##         bic <- log(n)*fpar*length(x$best$res)*nrow(x$data) - 2*max(x$best$ll)
        ##         bic <- log(n)*fpar*length(x$best$res) - 2*max(x$best$ll)
        ##     }
        ##     return(bic)
        ## }
        ## res <- list()
