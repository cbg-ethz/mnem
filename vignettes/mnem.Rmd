...
set.seed(9247)    
simmini <- simData(Sgenes = 4, Egenes = 10,
                  Nems = 3, mw = c(0.3, 0.3, 0.4), nCells = 100, uninform = 2)
data <- simmini$data
ones <- which(data == 1)
zeros <- which(data == 0)
data[ones] <- rnorm(length(ones), 1, 0.5)
data[zeros] <- rnorm(length(zeros), -1, 0.5)
...
We forget the solution stored in 'simmini$Nem' and use the 'mnem' function to
   ...
   our causal networks with $k=2$. We have almost found the same causal
   networks we used to generate the data. We got only one false positive edge
   ($2$ to $1$) in the red network. Notice, that our input for the
...
the three component model gives us the best score (minimum). The five component
model is now also overtaken by the two component model due to complexity or lack
...
is expected due to the relatively low level of noise. However, almost $20$ cells
have responsibilities around $50\%$. This is not unexpected either. Both the
red and blue networks put the knock-down of S-gene $2$ as a key regulator
most upstream. Thus both networks predict the same profile for cells perturbed
by a knock-down of S-gene $2$. The mixture weights are
responsible for the slight shift of cell responsibilities from around $57$ and
...
result_disc <- mnem(datadisc, k = 3, search = "greedy", runs = 10,
                        method = "disc", fpfn = c(0.05,0.1))
plot(result_disc)
```

Apart from slightly different mixture weights we get the same signaling
networks. However, notice also the difference in E-gene distribution; at
least for the blue and brown networks.
