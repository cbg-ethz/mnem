...
for i in `seq 1 100`; do
    ...
    echo "module load R/3.1.0" >> $file
    echo "R --slave 'Sgenes=3' 'run=$i' 'noise=1:4' 'nem=1:2' 'hiddenstate=0' < mnem.R" >> $file
    qsub -q mpi04-ht.q -l mf=500M $file
    sleep 1
    rm $file
done
...
for (run in 1:length(acc)) {
    ...
    if (length(mw) == length(res$mw)) {
        mwcor[run] <- cor(sort(res$mw), sort(mw))
    }
    if (length(mw) > length(res$mw)) {
        mwcor[run] <- cor(sort(c(res$mw, rep(0, length(mw) - length(res$mw)))), sort(mw))
    }
    if (length(mw) < length(res$mw)) {
        mwcor[run] <- cor(sort(c(mw, rep(0, length(mw) - length(res$mw)))), sort(res$mw))
    }
    for (i in 1:length(res$limits)) {
         print(res$limits[[i]]$ll)
    }
    ...
    randfull <- NULL
    for (i in 1:nem) {
        tmp <- matrix(sample(c(0,1), Sgenes*Sgenes, replace = TRUE), Sgenes, Sgenes)
        tmp[lower.tri(tmp)] <- 0
        rownames(tmp) <- colnames(tmp) <- sample(1:Sgenes, Sgenes)
        tmp <- tmp[order(rownames(tmp)), order(colnames(tmp))]
        tmp <- transitive.closure(tmp, mat = TRUE)
        randfull <- cbind(randfull, t(tmp))
    }
...
for (i in 1:10) {
    ...
    start <- Sys.time()
    ## source("~/Documents/mnem/R/mnems.r"); source("~/Documents/mnem/R/mnems_low.r")
    init <- modules(data, modulesize = 4, search = "greedy", verbose = TRUE)
