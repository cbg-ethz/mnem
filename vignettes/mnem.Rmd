...
(right, red). Notice that the raw log likelihood of $k=4$ is higher than
for the $k=3$ component model. However, if we penalize complexity
the three component model gives us the best score (minimum).
The penalized like the raw log likelihood is not guaranteed to have
only one global optimum, but can have several local ones. Hence, in practice
when there are little time constraints larger $k>4$ should also be
tested.
...
```{r, fig.height=4, fig.width=6, fig.cap=fig.cap0em}
plotConvergence(bestk$best)
```
...
result <- mnem(data, k = length(mw), search = "greedy", multi = TRUE,
               starts = starts)
plot(result)
```
