...
for (run in 1:length(acc)) {
    ...
    if (FALSE) {
        ...
        mw2 <- apply(getAffinity(p, norm = TRUE, mw = mw), 1, sum) # apply(2^p, 1, sum)
        mw2 <- mw2/sum(mw2)
        ...
        for (i in 1:3) { # i == 1 seems to win most of the time
            ...
            probs <- getProbs(probs, k, data2, gtn, n=n)
        ...
        p3 <- getAffinity(p, norm = TRUE, mw = mw2)
        p4 <- getAffinity(probs, norm = TRUE, mw = mw2)
        print(getLL(probs))
...
for (i in 1:runs) {
    ...
    data <- sim$data
    data2 <- sampleData(sim$Nem[[1]], Sgenes*Egenes, uninformative = (Sgenes*Egenes*0.1), type = "density", replicates = 3, meansH1=rep(1, ncol(Phi)), meansH0=rep(-1, ncol(Phi)), sdsH1=sample(seq(0.1,1,by=0.1),ncol(Phi),replace=TRUE), sdsH0=sample(seq(0.1,1,by=0.1),ncol(Phi),replace=TRUE))$D
    if (bin) {
        ratio <- FALSE
        fpfn <- sample(1:length(data), floor(length(data)*binoise))
    ...
    if (!bin) {
        data <- data + rnorm(length(sim$data), 0, noise)
    }
    if (!is.null(beta)) {
        noise <- beta
        data <- sim$data
        data[which(data == 1)] <- log(rbeta(sum(data == 1), noise, 1)/rbeta(sum(data == 1), 1, noise))
        data[which(data == 0)] <- log(rbeta(sum(data == 0), 1, noise)/rbeta(sum(data == 0), noise, 1))
        HeatmapOP(data)
    }
    start <- Sys.time()
    ## source("~/Documents/mnem/R/mnems.r"); source("~/Documents/mnem/R/mnems_low.r")
    init <- NULL
    res <- mynem(data, start = init, search= c("modules", "greedy"), parallel = NULL, ratio = ratio, modulesize = 4)#, prior = sim$Nem[[1]])
    print(time <- time + Sys.time() - start)
    ...
    lines(i, (tp+tn)/(tp+tn+fp+fn), col = 6, pch = 6, type = "b")
    sham <- sham + shd(res$adj, sim$Nem[[1]])
    print(sham)
    start <- Sys.time()
    n <- getSgeneN(data)
    if (ncol(data) > Sgenes) {
    ...
    sham2 <- sham2 + shd(res$adj, sim$Nem[[1]])
    print(sham2)
    if (!is.na(tp/(tp+fn))) {
...
dowhat <- 3 # dimnames(simres3)

pdf("sim.pdf", width = 8, height = 6)

mar <- 1.5

par(mfrow=c(5,4),mar=rep(mar, 4),oma=rep(mar,4))
...
for (i in 1:5) {
    for (j in c(3,5,10,20)) {
        ...
        for (k in c(1,2)) {
            for (l in 1:3) {
                tmp3 <- tmp[,l,i,k,dowhat]
                tmp3[which(tmp3 == 0)] <- mean(tmp3[which(tmp3 != 0)])
                tmp2 <- cbind(tmp2, tmp3)
            }
        }
        boxplot(tmp2, col = c(rep("red", 3), rep("blue", 3), rep("grey", 3)), xaxt = "n", ylim = c(0.5,1))
        axis(1, 1:(length(noises)*3), rep(noises, 3))
        ## axis(1, c(length(noises)/2 + 0.5, length(noises)/2*3 + 0.5, length(noises)/2*5 + 0.5), c("mnem", "nem", "random"), line = 2, tick = FALSE, cex = 2)
...
pdf("sim_k.pdf", width = 10, height = 14)
...
for (i in 1:5) {
    for (j in c(3,5,10,20)) {
        ...
        boxplot(tmp2, col = rep("red", 3), xaxt = "n")
        axis(1, 1:(length(noises)*3), rep(noises, 3))
...
for (i in 1:100) {
    for (j in 1:5) {
        for (k in 1:3) {
            if (!file.exists(paste0("temp/simres_mnem_", Sgenes, "_", i, "_", k, "_", j, ".rda"))) {
                stop("got one")
                file <- paste0("MissedIt", Sgenes, "_", i, "_", k, "_", j, ".sh")
                system(paste0("file=", file))
                system(paste('echo "module load repo/grid" >> ', file))
                system(paste('echo "module load grid/grid" >> ', file))
                system(paste('echo "module load gcc/5.3.0" >> ', file))
                system(paste('echo "module load R/3.4.0" >> ', file))
                tmp0 <- paste0("'Sgenes=", Sgenes, "' 'run=", i, "' 'noise=", k, "' 'nem=", j, "'")
                tmp <- paste0('echo "R --slave --args ', tmp0, ' < mnem/vignettes/sim.R" >> ', file)
                system(tmp)
                system(paste0("qsub -q mpi04-ht.q -pe make 1 ", file))
