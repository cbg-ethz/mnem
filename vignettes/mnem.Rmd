...
```{r plot, fig.width = 4, fig.height = 4, warning=FALSE, fig.cap = "Random mixture weights. Distance between groudn truth and random mixture weights for different numbers of components."}
runs <- 100
maxk <- 5
...
for (j in 1:3) {
    ...
    i2 <- which.min(bics[-which.min(bics)])
